/**
 * Tests for FragmentOverlay — Step 10 data-layer and projection.
 *
 * Covers:
 *  - Existing container/children behaviour (unchanged).
 *  - Stored-fragment projection: brackets rendered at projected positions when
 *    fragments + ghostLayer are provided.
 ...
 *  - Click target fires onBracketClick with the fragment id.
...
/** A minimal FragmentListItem for a measure-level fragment. */
...
/** Ghost layer covering bars 1–4, all on one system row (systemTop=50). */
...
describe('FragmentOverlay — projection', () => {
  ...
  it('projects correct left and width from ghost layer bounds', () => {
    // Fragment covers bars 2–3: left=100, right=400 (100+100+100+100 but only
    // bars 2-3 → left=100, right=300 → width=200).
  ...
  it('re-projects when ghostLayer prop changes (zoom/resize re-render)', () => {
    ...
    // Initial: left=0, width=200 (bars 1-2, 100px each)
...
describe('FragmentOverlay — pointer-events', () => {
  it('bracket bar has pointer-events: none so ghost drags pass through', () => {
    ...
    // The CSS module sets pointer-events: none; jsdom computed style returns ''
    // for CSS module classes, so assert the class is applied rather than the
    // computed property (which is the convention for CSS module tests here).
    expect(bar.className).toContain('storedBracket');
  });

  it('click target button is rendered on the first (and only) segment', () => {
    const frag = makeFragment('frag-ct', 1, 2);
    render(
      <FragmentOverlay
        fragments={[frag]}
  ...
  it('click target calls onBracketClick with the fragment id', async () => {
    ...
    const handler = vi.fn();
    const frag = makeFragment('frag-click', 1, 2);
    render(
      <FragmentOverlay
        fragments={[frag]}
        ghostLayer={FOUR_BAR_LAYER}
  ...
  it('no click target is rendered when onBracketClick is not provided', () => {
    const frag = makeFragment('frag-no-click', 1, 2);
    render(<FragmentOverlay fragments={[frag]} ghostLayer={FOUR_BAR_LAYER} />);
    expect(
