...
**Repeat sections — ghost index collision.** Measures in first and second endings share `@n` values in MEI. The flat `measureIndex` and `beatIndex` arrays would map both endings to the same slot, with the second rendering overwriting the first. **Fix:** ghost IDs and index keys must incorporate ending context (e.g. `m${n}-e${endingN}`) when a measure falls within a `<ending>` element. The MEI parser must detect first/second ending context during ghost construction by walking up the DOM to the containing `<ending>` and reading its `@n`.
