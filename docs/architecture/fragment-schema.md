...
**`repeat_context`** is a display-only field that records which repeat ending a fragment falls in: `first_ending`, `second_ending`, or further values as they arise. Null means the fragment does not fall inside any ending. It is no longer used for rendering disambiguation — `mc_start`/`mc_end` already identify the physical measures unambiguously without it.

> **Updated 2026-07-26 (Component 11 Step 10 / M6).** The "cleaner long-term approach — filtering harmony events by `mc` range directly" that this paragraph deferred to Component 3 **has landed**, brought forward because it was not merely cleaner but necessary: on a movement whose bar numbers restart, the `mn` range matches both passes, and on K331/ii the MEI's `@n` and the DCML `mn` disagree outright (the MEI restarts at the Trio, the annotation numbers through), so a Trio fragment was served the *Menuetto's* harmony. `_slice_harmony_events`, `_sources_in_range`, and the `GET /analysis/events` endpoint now match on `mc_start <= mc <= mc_end` for any event carrying an `mc`; the `(mn, volta)` test, and with it `repeat_context`'s role in this query, remains only as the fallback for an event with no `mc` (manually inserted ones — `mc` is optional on the harmony API payloads). See ADR-015 and ADR-036.

**`section_label`** is not a stored column: it is resolved on read from the `movement_section` table by `mc` containment and returned on the fragment read models (`FragmentDetailResponse`, `FragmentListItem`, `ReviewQueueItem`, `ConceptBrowseItem`). It names the movement section a fragment begins in ("Trio") and is non-null only for movements whose notated bar numbers restart — K331/ii, where "mm. 12–15" would otherwise name two different places in the score. Resolution is server-side because `ConceptBrowseItem` (the public browse card and glossary example caption) carries no machine coordinate to resolve it from. Display convention, including the volta suffix rendered from `repeat_context`, is in [ADR-036](../adr/ADR-036-movement-sections-and-bar-label-disambiguation.md).

**`parent_fragment_id`** records sub-part nesting. The database imposes no depth limit: nesting may be arbitrarily deep when it is conceptually valid (a composite concept whose stages themselves have stages). The tagging UI performs visual flattening beyond two visible levels so the score does not become unreadable, but the data model preserves the true hierarchy. A sub-part's extent must fall within its parent's; this constraint is enforced by the service layer, not the database. The comparison is on **`(mc, beat)` position pairs**, not on bar numbers: `mc` is the only unambiguous measure coordinate (bar numbers repeat across sections and endings, so `bar_start` alone cannot order two positions in K331/ii), and within a measure the beats order the pair, with a null beat meaning that measure's own edge — which is why no meter is needed to compare them. *(Corrected 2026-07-27, Component 11 Step 11: the check was bar-only, and its stated reason for excluding beats — "beat values are measure-local" — only holds for beats stripped of their measure. That gap is how M7's whole-measure stage bounds were written under a fragment starting at beat 3.)*
...
**`music21_version`** *(string, required when any auto field is present)*
The version of music21 used to generate the auto-extracted fields (e.g. `"9.1.0"`). Stored so records generated by an older version can be identified and re-processed after a library upgrade that changes analysis output. In the DCML-only path (Component 5, option b) where no music21 analysis runs, this field stores the sentinel `"none"` or `null`; the Pydantic model accepts both.
...
2. Every harmony event in `movement_analysis` whose position falls within the fragment's `[bar_start, beat_start] .. [bar_end, beat_end)` range must have `reviewed: true` — but only if the fragment's concepts include at least one that requires harmony review. For fragments whose concepts do not capture harmony (e.g. a Hemiola), this check is skipped; the harmonic context is still displayed when rendering the fragment, but unreviewed events do not block approval. **As built (updated 2026-07-26, Component 11 Step 10 / M6):** the range query matches on `mc` (`event.mc >= mc_start AND event.mc <= mc_end`) for every event carrying one, which is unique per ending and per section and so needs no `volta` filter; the `(mn, volta)` test remains only for an event with no `mc`. The beat bounds clip the boundary measures, so the gate now checks exactly the `[bar_start, beat_start] .. [bar_end, beat_end)` range this paragraph specifies rather than whole measures — it previously demanded review of events outside the fragment. The "deferred to Component 3" simplification described here has therefore landed early: it turned out to be necessary, not merely cleaner, because on a movement whose numbering restarts the `mn` range matches both passes and, on K331/ii, the MEI's `@n` and the DCML `mn` disagree outright. See ADR-015 and ADR-036.
