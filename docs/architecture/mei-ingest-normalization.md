...
> **Correction (2026-07-25).** *The ingested corpus does not follow this convention, and never has.* A survey of all 54 movements ([`step-9a-duplicate-bar-number-survey.md`](../reports/component-11-reports/step-9a-duplicate-bar-number-survey.md)) found nine movements with `<ending>` elements and **no `@n` shared across sibling endings in any of them**. The second ending consistently carries a **DCML X-prefixed complement** — K331/i: `mc 98 @n="98"` (first ending) then `mc 99 @n="X1"` (second) — because these are split-measure complements at a repeat boundary, which is how the source encodes them. So no volta produces an ambiguous bar label, and `repeat_context` is disambiguating something that was never ambiguous (it remains useful as musical information, and is displayed as prose per ADR-036).
>
> Two consequences follow, neither yet actioned. The "Doppia requires integer `@n` on all measures" rule above is **not enforced in practice** — X-prefixed values are accepted with an `info` advisory (`MEASURE_N_DCML_X`, 16 movements). And because `fragment.bar_start`/`bar_end` are `INTEGER`, a selection beginning on an X measure — which every second ending in this corpus is — has no faithful human coordinate to store; no such fragment exists today, and the gap is tracked as `phase-2.md` Track M **M16**.
