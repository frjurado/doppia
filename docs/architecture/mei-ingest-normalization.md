...
### Finding 5 — Adopted approach: `breaks="smart"`, narrow `pageWidth`, take page 1

Since `measureRange` selection is non-functional, the incipit is generated by rendering the full score with a narrow `pageWidth` that causes Verovio's smart line-breaking algorithm to fit roughly the first system onto page 1. Page 1 is then the incipit.
...
tk.loadData(mei_bytes.decode("utf-8"))
tk.select({"measureRange": f"{start_pos}-{end_pos}"})  # 1-based position index
tk.redoLayout()
svg = tk.renderToSVG(1)
```

`start_pos` and `end_pos` are position indices (not `@n` values). The fragment renderer must convert `bar_start`/`bar_end` (which are `@n` values from the database) to position indices before calling `select`. The simplest conversion: load the MEI, walk the measures in document order to build a `{@n_value: position_index}` map, then use that map when constructing the range.
