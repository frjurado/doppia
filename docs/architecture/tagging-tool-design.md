...
- **There is no hard clamp on the main-bracket drag.** It was removed in Component 11 Step 13: `respondToMainResize` already absorbs a shrink correctly at every resolution, and where the two disagreed the clamp was the cruder instrument — it protected whole *bars*, so a stage occupying one beat of m. 10 made the whole of m. 10 unremovable. When a shrink genuinely leaves no room, `respondToMainResize` returns `blocked` and the submission checklist says so; the bracket still moves.
...
#### Why the main-bracket drag has no clamp (Component 11 Steps 12–13)

The main bracket moves freely; `respondToMainResize` absorbs whatever the move implies. Getting here took two passes, and the intermediate state is worth recording because the first fix looked complete and was not.

The original hard clamp (`computeResizeClamp` → `AnnotationSession.setMinBarRange`) pinned the drag to the bar span of every *confirmed* stage. Editing a stored fragment was therefore impossible: restoring one marks every stage confirmed so the sidebar does not accuse a settled fragment of being in limbo, and the clamp read those same flags as "pinned by hand". The stages fill the fragment exactly, so there was nowhere left to shrink to. **Step 12** split the flag in two — `confirmed` for limbo, a new `anchored` for the clamp — so restored stages stopped blocking.

That fixed the reported case and left the mechanism intact, which turned out to be the real fault: as soon as an annotator dragged *any* stage handle or re-enabled an absent stage, that stage became anchored and the clamp came back for the rest of the session. And because the clamp reasons in **bar numbers** while stage bounds became beat-precise in Step 11, a stage occupying one beat of m. 10 protected the whole of m. 10 — the endpoint could move inside that measure but never leave it. ("At least a beat fights to keep the measure included.")

So **Step 13 removed the clamp entirely**, along with `anchored`, which had no other reader. Nothing is lost: `respondToMainResize` rebuilds the frame at the correct resolution, pins the boundaries whose slots survive, redistributes the rest, and reports `blocked` when even sub-beat resolution cannot seat the stages — at which point `computeStagesComplete` is false and the checklist says so. A required stage left with an empty run carries `error: true` (Step 11). Every degenerate outcome the clamp existed to prevent is prevented downstream, at every resolution, which the bar-granular clamp never managed.
...
*As implemented (Component 9 Step 4, `stageFrame.ts`):* the stage layout frame is a **slot list** — one slot per grid unit of the selection's effective measure-key range at the active resolution (`buildStageSlots()`), with the selection's beat-precision endpoint filters applied, so the frame's ends are the main bracket's exact endpoints.

> **I7 at measure resolution (corrected, Component 11 Step 11).** The endpoint filters were applied at beat and sub-beat resolution only; measure slots spanned their whole measure. But the stage grid is chosen from the *stage count*, not from the fragment's precision — a beat-precise fragment whose stages fit measure-granularly is the normal case — so I7 was quietly false exactly there, and the first and last stage of "m. 8 beat 3 – m. 10 beat 1" (279/ii) covered whole measures and overflowed their parent fragment: in the stage brackets, in the sidebar, and in the stored sub-part rows. The measure branch now clips both endpoint slots to the selection's beat bounds, in geometry *and* in beat coordinates, and drops an endpoint measure the bounds leave uncovered — the same reduction `formatFragmentRange` makes for display. `prePopulateStages` pins its outer edges the same way; existing rows were repaired by `backend/data_migrations/clamp_subpart_bounds.py`. K active stages are a partition of the slot list by K−1 interior boundary indices: the boundary index *is* the single shared value of the overlap-prohibition rule, and I7/I8 are properties of the partition rather than re-established post-conditions. `moveBoundary()` implements I6/I9/I10 as a total pure function (every cursor position maps to a legal clamped boundary vector; overtaken optional boundaries ride along with the drag; required stages bound the target by one slot). `StageBrackets` re-derives the displayed assignments from the gesture's *initial* frame on every drag tick, which is what makes mid-gesture collapse restoration (I10) automatic. Stage geometry, like the main bracket's, never consults `@n` intervals: a stage's bounds carry the physical measure keys of both boundaries (`StageBounds.keyStart`/`keyEnd`), and duplicate bar numbers elsewhere in the movement contribute nothing to a stage's pixels (STG-10) or its sub-part `mc` payload.
...
- A confirmed/active stage on the side **opposite** the dragged main handle keeps its absolute bounds, except where the finer-grid escape valve forces proportional resizing — and then I7/I8 still hold.
...
**Containment constraint enforcement**: the server validates that every child fragment's spatial bounds fall within the parent fragment's bounds. This is a service-layer check (not a database constraint) applied before the transaction begins. It compares `(mc, beat)` position pairs — see `fragment-schema.md` § `parent_fragment_id` for why that is both meter-free and the only ordering that survives duplicate bar numbers. *(It was bar-only until Component 11 Step 11; that is how M7 reached the database.)*
...
| §6 Hybrid main-resize response (Step 3; clamp removed in Component 11 Step 13) | `frontend/src/components/score/stages.ts`, `frontend/src/components/score/StageBrackets.tsx`, `frontend/src/components/score/StageList.tsx` |
