...
- **The resize hard-clamps to protect stages the annotator just placed.** The main-bracket drag cannot shrink past the point that would force an *anchored* stage below one grid unit. The clamp fires only after the finer-grid escape valve is exhausted.
...
#### `confirmed` vs `anchored` (Component 11 Step 12)

A stage assignment carries **two** flags where it used to carry one, because "this bracket's position is settled" and "the annotator pinned this bracket here, just now" are different claims and only the second should stop a drag:

| Flag | Means | Read by |
|---|---|---|
| `confirmed` | The position is settled, not a default awaiting review. Set by a drag **and** by restoring a stored fragment. | limbo warnings (§4 "Optional stages"), boundary pinning in `respondToMainResize` |
...
The one flag did double duty, and editing a stored fragment paid for it: restoring one marks every stage confirmed so the sidebar does not accuse a settled fragment of being in limbo, and the clamp then read those same flags as "pinned by hand" and refused to shrink the main bracket at all — the stages fill the fragment exactly, so there was nowhere to shrink to. Shrinking only worked if the outermost stage was dragged in first. (Reported as the M0 carryover; resolved as Component 11 Step 12, Option 1.)

Pinning deliberately stays on `confirmed`. A restored stage must not *block* a shrink, but while its slot survives the resize it should stay exactly where its annotator put it — and the ghost layer is rebuilt with a fresh selection object on every re-render, zoom, and progressive page load, so pinning on `anchored` would redistribute a stored fragment's stage boundaries on a plain browser zoom. The two together give Option 1's promise ("redistribute like creation") without its stated cost: no restored stage reads as needing re-confirmation, because `confirmed` is untouched.
