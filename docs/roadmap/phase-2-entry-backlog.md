...
| **Public endpoints + ADR-009 enforcement** | The unauthenticated `approved`-only browse path and the ABC-corpus exclusion. **Backend landed** (Component 10 Steps 3–4): the `/api/v1/public/` router (`071a525`) serves anonymous `approved`-only browse/detail with a path-scoped CORS split, and the ADR-009 NonCommercial-corpus exclusion is enforced structurally in the service (keyed on corpus licence, proven by an ABC fixture test). **Still open:** the public frontend read views (Step 5). | ADR-009 § Implementation note; `phase-1.md` Component 8 § Phase 1 scope |
