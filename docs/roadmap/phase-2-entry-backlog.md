...
| ~~**OpenAPI docs exposure**~~ | ✅ Landed `0fcde03` (Component 10 Step 10, 2026-07-22): `create_app()` disables `/api/docs`, `/api/redoc`, `/api/openapi.json` in production (`docs_url`/`redoc_url`/`openapi_url=None` when `ENVIRONMENT=production` → routes 404); reachable in local/staging for development. | `security-model.md` § 7 (shipped) |
