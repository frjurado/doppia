...
- Corpus setup correction: if measure numbers in an uploaded MEI file pass validation but violate the project's conventions (e.g. a pickup bar numbered 1 instead of 0, or numbers following a different edition's system), provide a correction tool — a Python script with a dry-run mode — that renumbers measures in the MEI XML and updates any fragment pointers atomically. This is a **pre-tagging corpus setup utility**: the workflow is to catch and fix convention mismatches during initial ingestion, before annotation begins. If numbering errors are discovered after substantial tagging, that warrants re-tagging rather than automated correction. The atomic fragment-pointer update is a safeguard for the case where a small amount of tagging occurred before the error was spotted. **Verify the correction script before populating any production fragments.** The test should: upload a file with non-standard measure numbers, create a fragment pointing to bars 3–5, run the correction, verify the fragment pointer updated correctly.
...
**Rhythmic subdivision grid:** selection at beat or sub-beat precision was scoped for a future phase. Sub-measure precision is significantly more complex than measure-level selection and requires extending the fragment coordinate system. **Decision (ADR-005):** implement measure-level precision in Phase 1; extend to beat-level in Phase 2. The `fragment` table includes `beat_start` and `beat_end` (nullable float columns, in beats from the beginning of the start/end measure) from day one, even though Phase 1 leaves them null. See also ADR-011 for the multi-level tagging design decisions.
