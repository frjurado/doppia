...
Several scoping decisions are baked in. The first four are sensible defaults consistent with the design docs; the fifth — the rendering-context contract — is the substantive one and is surfaced in "Decisions to Confirm" because it amends a forward-compatibility note already written into `phase-1.md`.
...
`GET /api/v1/fragments?concept_id={id}&include_subtypes={bool}&status={status}`, `require_role("editor")`, cursor-paginated per the API conventions. The two-step cross-database join is the canonical pattern from `fragment-schema.md`: (1) resolve the concept id set via `get_subtype_ids` (Step 1), (2) query PostgreSQL for fragments whose `fragment_concept_tag.concept_id` (with `is_primary = true`, so a fragment surfaces under its driving concept, not its cross-reference tags — confirm this is the desired browse semantics) is in that set. The join is owned by the service layer; the route handler calls the service.
...
- **Rendering-context API contract.** `phase-1.md` § Component 8 specifies an optional `context_bars` integer (default 0) so a future feature can render additional surrounding bars without a data-model change. The intent is right; the *shape* is too narrow — a single symmetric int cannot express the contexts that are actually wanted (more bars *before* than *after*; the enclosing container fragment; the stretch since the previous same-domain fragment). Replace the bare int with a structured, default-preserving `context` parameter on the detail read:
  ...
  Because this amends a forward-compat note already written into `phase-1.md`, it is flagged in "Decisions to Confirm" — the bare-int contract is a published shape, and changing it (even before any consumer exists) is Francisco's call. If he prefers to keep `context_bars` as written, the fallback is to keep the int but document explicitly that it is symmetric and that asymmetric/fragment-relative context is a known future need requiring a contract revision.
...
- `docs/roadmap/phase-1.md` § Component 8 — replace the bare `context_bars` forward-compat note with the structured `context` contract (Step 12), pending the "Decisions to Confirm" sign-off; add a short note that browsing ships annotator-facing (editor-role) in Phase 1 with the public `approved`-only path deferred to Phase 2, and that the `data_licence` serialiser is built here ahead of that public path.
- `docs/architecture/fragment-schema.md` — confirm the status-filter-at-service-layer wording covers the new concept-scoped list/browse endpoints; note the concept-scoped browse query as the reusable shape Phase 3 retrieval reuses (cross-reference the forward-compat note) and that `is_primary` governs which concept a fragment surfaces under in a browse.
...
- A new ADR **only if** Francisco accepts the structured `context` contract over the published `context_bars` int (Step 12 / Decisions to Confirm) — the change to a published forward-compat shape is exactly the kind of non-obvious decision CLAUDE.md's Definition of Done requires recording. If accepted, add `docs/adr/ADR-024-fragment-rendering-context.md` (or fold it into an existing rendering ADR); if declined, document the symmetric-int limitation in place.
...
- **Rendering-context contract (Step 12).** This plan replaces the published `context_bars` integer (`phase-1.md` § Component 8) with a structured `context` object carrying a `mode` (`none` default; `bars` with asymmetric `before`/`after`; `enclosing_fragment`; `previous_same_domain`), of which Phase 1 implements only `none`. The motivation is that a single symmetric int cannot express the contexts actually wanted (asymmetric surrounding bars, the enclosing container fragment, the span since the previous same-domain fragment) without a later breaking change to a published contract. The default behaviour is unchanged either way. Confirm the switch to the structured contract (and whether to record it as ADR-024), or instruct keeping the `context_bars` int as written — in which case the symmetric-only limitation is documented as a known future-revision point rather than designed around now.
- **Browse surfaces by primary concept (Step 2).** The concept-scoped browse matches on `is_primary = true` tags, so a fragment appears under its driving concept and not under every cross-reference tag. Confirm this is the intended browse semantics (the alternative — surfacing a fragment under any of its tags — is a one-line query change but changes result sets and counts).
- **Public read path stays out of Component 8.** All reads are `require_role("editor")`; the ADR-009 `data_licence` serialiser and ABC-corpus exclusion are built/recorded but the unauthenticated `approved`-only endpoint is Phase 2. Confirm browsing is annotator-facing only in Phase 1 (consistent with Component 7), or flag if an unauthenticated browse is wanted earlier.
...
6. The rendering-context contract is settled per "Decisions to Confirm" and reflected in `phase-1.md` (and an ADR if the structured contract is accepted); Phase 1 implements only the default and validates-but-ignores the rest.
