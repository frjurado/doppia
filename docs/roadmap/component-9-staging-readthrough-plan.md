...
- [~] B1–B2 **traced & classified** (2026-06-28, `accidentals-playback-findings.md`); the Verovio-behaviour check is settled — Verovio realises each note from its own `accid`/`accid.ges` only (no staff-scope, no render-time running/key-sig inference), so every symptom is a converter `accid.ges` error and the resolution pass **is** needed. Still to do: implement the staff+octave-scoped, section-aware, onset-ordered resolution pass (Pass 9 extension), resolve the override-vs-add design question, promote `scripts/accidental_trace.py`.
