...
| C2 | Trio repeat structure — first repeat jumps to minuet `|:` (m.19); missing start-repeat; da-capo `<i>`; blank space m.40 | C | Overlay (start-repeat, ✔ visual) + **playback likely fixed by Phase B (B2)** — same restarting-`@n` root as the clef collision; verify at B2 + Step 15 (display) | ◐ **repeat structure now coherent (2026-07-01, ADR-032).** With unique-number import the repeat marks land on the right measures: Menuetto `|: mc1-18 :| |: mc19-48 :|`; **Trio `rptend@mc64 ↔ rptstart@mc65 (C2 errata) ↔ rptend@mc100`** — the Trio repeat now closes on the *Trio's* own start-repeat, not the Menuetto's `rptstart@mc19` (the "jumps to minuet" symptom). Warning set unchanged from the D3 ingest (no regression). **Trio "blank space" (the stray extra space around m.40) is GONE** — confirmed on the 2026-07-01 live render, another facet of the duplicate-`@n` import collision resolved by the renumber. **Still broken: rendered MIDI playback** — Francisco confirms in-app (2026-07-01) that *both* Trio repeat-ends still jump back to Menuetto m19, even with the structure now correct; under active investigation (Verovio repeat-expansion, separate from the MEI structure). Da-capo `<i>` → Step 15 / multi-section cluster. |
