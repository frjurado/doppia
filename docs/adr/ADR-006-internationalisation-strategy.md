...
- ✅ Service layer applies the translation overlay (`backend/services/translation.py`) on all concept-returning reads (`ConceptService.search` / `get_schema_tree` / `get_roots` / `get_tree`). Valid languages are `{'en', 'es'}`.
- ✅ **Spanish content seeded** (Component 12 Step 19b, 2026-09-06). `'es'` now serves Spanish rather than exercising the fallback: 34 concepts, 13 schemas and 22 values, from `backend/seed/translations/es.yaml` — the seed file this ADR anticipated when it said adding a language is "a data migration and a seed file, not a code change". The file carries one `status` for the whole pass rather than a status per entry, because a translation pass is reviewed as a body of work: written at `machine`, promoted to `reviewed` after Francisco's revision by editing one line. `authoritative` stays reserved for content adapted to the Spanish-language theoretical tradition rather than rendered into Spanish. `source_hash` is deliberately **not** written for non-English rows: on an English row it baselines that row's own text, and the staleness job compares a translation against the *English* hash — writing the Spanish text's own hash there would make every translation look permanently current.
- ✅ Property value `short_name` and `description` are translatable (migration `0015`; see ADR-039 § 4). The overlay falls back **per field**, so a row that localises only the name keeps the English short form rather than blanking it.
...
- **Tree cache.** The Redis concept-tree cache key is language-scoped (`tree:v2:{root}:{language}`); the seed-time `tree:*` invalidation covers every suffix. The cached payload is the *translated, count-free* node structure: per-concept approved-fragment counts are read live from PostgreSQL on every request and never cached (Component 11 Step 8 / M11 — caching them made counts up to an hour stale). The `v2` segment was introduced by that fix so a pre-fix entry, whose payload embedded the counts, can never be read back.
