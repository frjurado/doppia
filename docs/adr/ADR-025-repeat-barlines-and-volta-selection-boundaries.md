...
- Fragment playback needs the final-pass rule before fragments spanning repeat-ends are auditioned (Component 9 playback work). Until then, such a fragment played via the default Verovio expansion could jump outside its own range. (As implemented in Step 18: *not* by stripping markup before `renderToMIDI()` as first guessed, but by **window selection over the expanded timemap**. `renderToMIDI()`/`renderToTimemap()` ignore the fragment `select()` and emit the whole movement, so `buildFragmentPlayback` reads the unrolled measure-onset sequence and windows from the *last* onset of `mc_start` entered from outside `[mc_start, mc_end]` to the first onset that leaves the range. This lands on the final pass for a truncated repeat-end while still expanding a wholly-contained repeat — no MEI/toolkit-state surgery. See `playback-coordinates.md` §"Fragment-scoped playback".)
