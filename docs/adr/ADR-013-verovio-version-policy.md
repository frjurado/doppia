...
**SVG structure (5.3, May 2025).** The way SMuFL font glyphs are embedded in the SVG output was refactored to improve compatibility with third-party SVG renderers. All snapshot baselines in `tests/snapshots/` are invalidated and must be regenerated. All incipit SVGs already stored in object storage were rendered under 4.3.1 and must be backfilled. *Note: the snapshot test suite in `backend/tests/snapshots/` was built in Component 10 Step 13 (2026-07-22) — `test_verovio_snapshots.py` pins the current 6.1.0 render (deterministic via `xmlIdSeed` + version-string strip, byte-stable across Windows/Linux) for a representative set (full movements, a mid-movement `select`, a first/second-ending volta) and runs in CI as its own step. **This is the M14 upgrade guard**: the deliberate 6.2.0 bump must run these; failures surface the rendering deltas for review, then are accepted by regenerating with `UPDATE_VEROVIO_SNAPSHOTS=1`. Before Step 13, the 6.1.0 upgrade (2026-04-23) was verified manually against the spike script in `docs/architecture/mei-ingest-normalization.md` §"Verovio bar-range selection: observed behaviour".*
