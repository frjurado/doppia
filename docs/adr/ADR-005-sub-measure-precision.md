...
| **Repeat sections — ghost index collision** | Not handled | Incorporate ending context into ghost IDs and index keys (e.g. `m${n}-e${endingN}`); detect `<ending>` context during MEI parse by walking the DOM. |
